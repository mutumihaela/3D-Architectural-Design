...
# # Set paths
# prompt_file = '/shared_storage/mutumihaela/shape-e-bun/prompts.txt'
# output_dir = '/shared_storage/mutumihaela/shape-e-bun/output'
# os.makedirs(output_dir, exist_ok=True)
...
# # Lower parameters to reduce memory usage
# batch_size = 1  # Keep at 1 to avoid VRAM overload
# guidance_scale = 5.0  # Lower this value to reduce memory load
# karras_steps = 6  # Reduce from 32 to save memory
...
# def render_mesh_open3d(mesh, save_path):
#     """
#     Render a 3D mesh using Open3D in offscreen mode (for headless environments).
#     """
#     try:
#         # Convert Open3D mesh to triangle mesh for rendering
#         mesh.compute_vertex_normals()
...
#         render.scene.camera.look_at([0, 0, 0], [0, 0, 1], [0, 1, 0])  # Look at the center

#         # Render image
#         img = render.render_to_image()

#         # Properly delete the renderer instance to free memory
#         del render  

#         # Convert Open3D image to PIL format and save
#         img_np = np.asarray(img)  # Convert to NumPy array
#         img_pil = Image.fromarray(img_np)
#         img_pil.save(save_path)
#         print(f"Rendered and saved image: {save_path}")
