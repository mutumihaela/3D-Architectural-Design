...
# # --- CONFIG ---
# OBJ_FOLDER = "/Users/mutumihaela/Desktop/blender/objects"
# OUTPUT_JSON = "/Users/mutumihaela/Desktop/blender/placement_plan.json"
# sentence = "Put a red painted vintage bookshelf on the left and a high back executive chair in the center."
...
# # --- Load prompts from filenames in folder ---
# obj_files = [f for f in os.listdir(OBJ_FOLDER) if f.endswith(".obj")]
# file_prompts = [os.path.splitext(f)[0].replace("_", " ") for f in obj_files]

# print("🧠 Encoding prompts from filenames...")
# prompt_inputs = clip_processor(text=file_prompts, return_tensors="pt", padding=True, truncation=True)
# with torch.no_grad():
#     prompt_embeddings = clip_model.get_text_features(**prompt_inputs)
...
#         print(f"[!] Low score ({best_score:.2f}) → skip")
#         return None
#     best_file = obj_files[best_idx]
#     print(f"✅ Matched with '{file_prompts[best_idx]}' ({best_score:.2f})")
#     return best_file
